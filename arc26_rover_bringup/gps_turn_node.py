...
class GpsTurnNode(Node):
    def __init__(self):
        super().__init__("gps_turn_node")

        self.declare_parameter("cur_lat", 39.815988)
        self.declare_parameter("cur_lon", 30.528493)
        self.declare_parameter("tgt_lat", 39.816100)
        self.declare_parameter("tgt_lon", 30.529000)
        ...
        self.cur_lat = float(self.get_parameter("cur_lat").value)
        self.cur_lon = float(self.get_parameter("cur_lon").value)
        self.tgt_lat = float(self.get_parameter("tgt_lat").value)
        self.tgt_lon = float(self.get_parameter("tgt_lon").value)

        self.target_heading = normalize_heading_deg(
            bearing_between_gps_deg(
                self.cur_lat,
                self.cur_lon,
                self.tgt_lat,
        ...
        self.target_heading_pub = self.create_publisher(Float32, "/navigation/target_heading_deg", 10)
        ...
        self.target_timer = self.create_timer(0.5, self.publish_target_heading)

        self.get_logger().info("GpsTurnNode started.")
        self.get_logger().info(f"Current GPS: {self.cur_lat:.8f}, {self.cur_lon:.8f}")
        self.get_logger().info(f"Target GPS : {self.tgt_lat:.8f}, {self.tgt_lon:.8f}")
        self.get_logger().info(f"Target bearing: {self.target_heading:.2f} deg")

    def heading_callback(self, msg: Float32):
        self.latest_heading = float(msg.data)
        self.latest_heading_time = time.time()

    def publish_target_heading(self):
        msg = Float32()
        msg.data = float(self.target_heading)
    ...
    def control_loop(self):
        ...
        self.log_throttled(
            f"GPS_AUTO heading={heading:.2f} target={self.target_heading:.2f} "
